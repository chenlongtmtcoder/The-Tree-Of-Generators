function exponentialFormat(num, precision, mantissa = true) {
  ...
  if (mantissa) return m.toStringWithDecimalPlaces(3) + "e" + e;
  else return "e" + e;
}
...
function format(decimal, precision = 3, small) {
  ...
  if (decimal.mag == Number.POSITIVE_INFINITY) return "F1.80e308";
  if (decimal.gte("eee1e10")) {
...
function formatTime(s) {
  if (s < 60) return format(s) + "s";
    ...
    return formatWhole(Math.floor(s / 60)) + ":" + format(s % 60);
  else if (s < 86400)
    return (
  ...
  else return formatWhole(Math.floor(s / 31536000)) + "years";
}
